...
```{r}
mean(gpa_study_hours$study_hours)

gpa_study_hours %>% 
  summarize (avg_StudyHrs = mean(study_hours))
```
...
```{r}
sd(gpa_study_hours$gpa)
sd(gpa_study_hours$study_hours)

 gpa_study_hours %>% 
  summarize(standardDevGpa = sd(gpa),
            standardDevStudyHrs = sd(study_hours))
```
...
ggplot(data = gpa_study_hours, mapping = aes(x = gpa))+
  geom_boxplot(color = "red",
                 fill = "orange", size = 1)
```
